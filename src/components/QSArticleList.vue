  ...
  <view class="article-list-container px-4 pb-4 bg-white">
    ...
    <view v-if="articles.length > 0">
      <view v-for="(article, index) in articles" :key="article.id"
        ...
        :class="[index !== articles.length - 1 ? 'border-b border-gray-100' : '']" @tap="openArticle(article)">
        <!-- 左侧：文字信息 -->
        <view class="flex-1 flex flex-col justify-between min-h-[140rpx]">
          ...
          <view class="flex flex-wrap gap-1.5 mt-2.5">
            <view v-for="tag in article.tags" :key="tag"
              class="tag-badge bg-[#fff0f0] border border-[#ffdbdb] rounded flex items-center justify-center p-1">
        ...
        <view class="image-wrapper shrink-0 w-[180rpx] h-[180rpx] rounded-lg overflow-hidden bg-gray-100">
          <image :src="article.thumbnail" mode="aspectFill" class="w-full h-full" />
        </view>
      </view>
    </view>
    ...
    <view v-else class="flex flex-col items-center justify-center py-12">
      <svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 text-gray-300 mb-2" fill="none" viewBox="0 0 24 24"
