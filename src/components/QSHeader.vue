<template>
  <view class="qs-header-container relative overflow-hidden" :style="{ paddingTop: safeArea.statusBarHeight + 'px' }">
    <!-- 红色渐变背景 -->
    ...
    <!-- 导航栏区域：左图标 + 标题 + 右图标 -->
    <view class="relative z-10 flex items-center justify-between px-4"
      :style="{ height: safeArea.capsuleHeight + 'px', marginTop: (safeArea.capsuleTop - safeArea.statusBarHeight) + 'px' }">

      <!-- 左侧：用户头像图标 -->
      <view
        class="header-icon-btn w-8 h-8 rounded-full border-2 border-white/60 flex items-center justify-center bg-white/10">
        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-white/90" fill="none" viewBox="0 0 24 24"
    ...
    <view class="relative z-10 mx-4 mt-3 mb-5">
      <view class="w-full bg-white rounded-full px-4 py-2.5 flex items-center shadow-sm">
        <!-- 搜索图标 -->
        ...
        <input type="text" :value="store.searchQuery" @input="onSearchInput" placeholder="搜索文章、作者、关键词"
          placeholder-class="text-gray-400 text-xs" class="w-full text-gray-800 text-xs outline-none bg-transparent" />
      </view>
    </view>
  </view>
</template>
