...
每次开发前必须先读本文件。开发完成后必须自审修改内容，并做类型检查、构建或必要的边界测试；不符合要求就继续修改，直到通过，中间有问题可集中提问再回答。
本项目是 uni-app + Vue 3 + TypeScript + Pinia + Tailwind + mock 微信小程序。
...
- 微信小程序优先使用 `view`、`text`、`image`、`button`、`input` 等 uni 组件；图标优先使用 uni-app 提供的图标。
- 对于二级页面的 header 优先使用组件 `src/components/SecondPageHeader.vue`。
